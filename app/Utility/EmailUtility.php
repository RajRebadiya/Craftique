...
class EmailUtility
{
    ...
    public static function selelr_registration_email($emailIdentifier, $user, $password = null){
        ...
        $emailBody = str_replace('[[date]]', date('d-m-Y', strtotime($user->created_at)), $emailBody);
        $emailBody = str_replace('[[admin_email]]', $admin->email, $emailBody);
        $emailBody .= self::sellerRegistrationSummaryBlock($user, $shop);
        $emailBody .= self::sellerRegistrationDetailsBlock($user, $shop);
    ...
    private static function sellerRegistrationDetailsBlock($user, $shop): string
    {
        $businessInfo = json_decode($shop->business_info ?? '', true) ?? [];
        $businessRoleLabels = [
        ...
        $fields = [
            ...
            'Registration Date' => !empty($user->created_at) ? date('d-m-Y', strtotime($user->created_at)) : '-',
            'What are you?' => $businessRoleLabels[$businessInfo['business_role'] ?? ''] ?? ($businessInfo['business_role'] ?? '-'),
            'Category' => $businessInfo['business_category'] ?? '-',
            'Products available' => trim(($businessInfo['product_quantity_range'] ?? '-') . (!empty($businessInfo['product_quantity_exact']) ? ' (' . $businessInfo['product_quantity_exact'] . ')' : '')),
            'E-shop domain' => $businessInfo['eshop_domain'] ?? '-',
            'Brand count' => $businessInfo['brand_count'] ?? '-',
            'Terms accepted' => !empty($businessInfo['accept_seller_terms']) ? 'Yes' : 'No',
        ];

        $html = '<br><hr><h3 style="margin:20px 0 10px;">Seller Registration Details</h3>';
        $html .= '<table cellpadding="6" cellspacing="0" border="0" style="width:100%; border-collapse:collapse;">';

        foreach ($fields as $label => $value) {
            $html .= '<tr>';
            $html .= '<td style="width:220px; font-weight:bold; vertical-align:top;">' . e($label) . '</td>';
            $html .= '<td style="vertical-align:top;">' . e($value) . '</td>';
            $html .= '</tr>';
        }
    ...
    private static function sellerRegistrationSummaryBlock($user, $shop): string
    {
        $businessInfo = json_decode($shop->business_info ?? '', true) ?? [];
        $businessRoleLabels = [
            'shop_creator' => 'Shop / Creator',
        ...
        $html = '<br><h3 style="margin:20px 0 10px;">Seller Registration Summary</h3>';
        $html .= '<ul style="margin:0 0 10px 20px; padding:0; line-height:1.6;">';
