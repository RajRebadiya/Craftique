...
function newBarcode() {
    ...
    var selectedProductName = selectedOption.text();
    var selectedProductPrice = selectedOption.data('price');
    var showProductName = $('input[name="showProductName"]:checked').val() === 'true';
    var showPrice = $('input[name="showPrice"]:checked').val() === 'true';
    var quantity = parseInt($("#quantity").val()) || 1;
    var perRow = parseInt($("#barcodes-per-row").val());
    var orientation = $("#orientation").val();

    var barcodeType = $("#barcodeType").val();
    var barWidth = parseInt($("#bar-width").val());
    var barHeight = parseInt($("#bar-height").val());
    var barMargin = parseInt($("#bar-margin").val());
    var backgroundColor = $("#background-color").val();
    var lineColor = $("#line-color").val();
    var displayValue = $(".display-text.btn-primary").val() === "true";
    var textAlign = $("input[name='text-align-options']:checked").val();
    var font = $("#font").val();
    var fontOptions = $(".font-option.btn-primary").map(function() { return $(this).val(); }).get().join(" ");
    var fontSize = parseInt($("#bar-fontSize").val());
    var textMargin = parseInt($("#bar-text-margin").val());
    ...
    for (let i = 0; i < quantity; i++) {
        ...
        JsBarcode(svg, selectedProductSlug, {
            format: barcodeType,
...
function calculateBarcodesPerPage(perRow, orientation) {
    return perRow * (orientation === 'horizontal' ? 10 : 15);
}

function createNewPage() {
    var page = $('<div class="a4-page" id="a4-page"></div>');
    $(".barcode-container").append(page);
    return page;
}

function createRow(page) {
    var row = $('<div class="d-flex"></div>');
    page.append(row);
    return row;
}
function printBarcodes() {
    var printContent = '';
    $('.a4-page').each(function() {
        printContent += '<div class="a4-page">' + $(this).html() + '</div>';
    });

    printJS({
        printable: printContent,
        type: 'raw-html',
        style: `@import url('https://cdn.jsdelivr.net/npm/bootstrap-colorpicker/dist/css/bootstrap-colorpicker.min.css');
                @import url('https://lindell.me/JsBarcode/generator/rangeslider.css');
                @import url('https://printjs-4de6.kxcdn.com/print.min.css');
                .a4-page { width: 210mm; min-height: 297mm; margin: auto; padding: 15mm; box-shadow: none; page-break-after: always; }
                .barcode-wrapper { box-sizing: border-box; text-align: center; padding: 10px; margin: 0 10px 10px 0; border: 1px dashed #000; }
                /* Include other styles that affect your barcodes and layout */`,
