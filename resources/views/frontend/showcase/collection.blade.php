...
@section('content')
    @php
        $pageTypeLabel = translate('Collection');
        $feedUrl = route('frontend.showcase.index', ['type' => 'collection']);
            ...
            <div class="bg-white border rounded shadow-sm p-4">
                <div class="row align-items-center">
                    <div class="col-lg-8 mb-3 mb-lg-0">
                        ...
                        <h1 class="h3 fw-700 mb-2">{{ translate('Collection Showcase') }}</h1>
                        <p class="text-muted mb-0">
                            {{ translate('Latest published Collection from the Showcase feed.') }}
                        </p>
                    </div>
    ...
    @if(!empty($item))
        ...
        @if(!empty($collectionItems) && $collectionItems->count())
            @include('frontend.showcase._collection_items', [
                'collectionItems' => $collectionItems,
                'sectionTitle' => translate('Collection Items'),
                'sectionSubtitle' => translate('Each card can contain its own cover, text and linked product.')
            ])
        @elseif(!empty($products) && $products->count())
            <section class="mb-5">
                <div class="container">
                    <div class="d-flex align-items-center justify-content-between flex-wrap mb-3" style="gap:12px;">
                        <div>
                            <h3 class="h4 fw-700 mb-1">{{ translate('Collection Products') }}</h3>
                            <p class="text-muted mb-0">{{ translate('Products linked with this Collection.') }}</p>
                        </div>
                    </div>
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body text-center py-5">
                        ...
                        <p class="text-muted mb-3">
                            {{ translate('There is no published Collection available right now.') }}
                        </p>
