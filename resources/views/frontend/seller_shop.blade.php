...
@section('content')
                        ...
                        <div class="d-flex justify-content-md-end">
                            ...
                            @if(in_array($shop->id, $followed_sellers))
                                <a href="{{ route('followed_seller.remove', ['id'=>$shop->id]) }}"
                                    data-toggle="tooltip" data-title="{{ translate('Unfollow Seller') }}" data-placement="top"
                                    class="btn btn-success d-flex align-items-center justify-content-center fs-12 w-190px follow-btn followed"
                                    style="height: 40px; border-radius: 30px !important;">
    ...
    @if (!isset($type))
        @php
            $storyFeedItems = \Illuminate\Support\Facades\DB::table('showcases')
            ...
            $collectionFeedItems = \Illuminate\Support\Facades\DB::table('showcases')
                ->where('seller_id', $shop->id)
                ->where('status', 'published')
        ...
        @if($storyFeedItems->count() > 0)
            <section class="mt-3 mb-3" id="section_story_feed">
                <div class="container">
                    <div class="d-flex mb-4 align-items-baseline justify-content-between">
                        <h3 class="fs-16 fs-md-20 fw-700 mb-3 mb-sm-0">
                            <span>{{ translate('Story Feed') }}</span>
                        </h3>
                        <div class="d-flex">
                            <a class="text-blue fs-12 fw-700 hov-text-primary" href="{{ route('frontend.showcase.history') }}">{{ translate('View All') }}</a>
                        </div>
                    </div>

                    <div class="aiz-carousel sm-gutters-16 arrow-none" data-items="6" data-xl-items="6" data-lg-items="5" data-md-items="4" data-sm-items="3" data-xs-items="2.2" data-arrows='true' data-infinite='false'>
                        @foreach ($storyFeedItems as $story)
                            @php
                                $storyTitle = $locale === 'en'
                                    ? (data_get($story, 'title_en') ?: data_get($story, 'title_gr') ?: data_get($story, 'title') ?: translate('Story'))
                                    : (data_get($story, 'title_gr') ?: data_get($story, 'title_en') ?: data_get($story, 'title') ?: translate('Story'));

                                $storyThumb = data_get($story, 'cover_image');
                                $storyThumbUrl = null;

                                if (!empty($storyThumb)) {
                                    if (is_numeric($storyThumb)) {
                                        $storyThumbUrl = uploaded_asset($storyThumb);
                                    } elseif (filter_var($storyThumb, FILTER_VALIDATE_URL)) {
                                        $storyThumbUrl = $storyThumb;
                                    } else {
                                        $storyThumbUrl = asset($storyThumb);
                                    }
                                }

                                $storyUrl = route('frontend.showcase.post', [
                                    'id' => data_get($story, 'id'),
                                    'slug' => \Illuminate\Support\Str::slug($storyTitle ?: data_get($story, 'id')),
                                ]);
                            @endphp

                            <div class="carousel-box">
                                <a href="{{ $storyUrl }}" class="d-block text-reset">
                                    <div class="text-center">
                                        <div class="mx-auto mb-2 overflow-hidden rounded-circle border" style="width:90px; height:90px; background:#f8f9fa;">
                                            @if($storyThumbUrl)
                                                <img src="{{ $storyThumbUrl }}" alt="{{ $storyTitle }}" class="img-fit w-100 h-100">
                                            @else
                                                <div class="d-flex align-items-center justify-content-center w-100 h-100 text-muted fs-12 fw-700">
                                                    {{ translate('Story') }}
                                                </div>
                                            @endif
                                        </div>
                                        <div class="fs-12 fw-600 text-truncate">{{ \Illuminate\Support\Str::limit($storyTitle, 18) }}</div>
                                    </div>
                                </a>
                            </div>
                        @endforeach
        ...
        @if($collectionFeedItems->count() > 0)
            <section class="mt-3 mb-3" id="section_collection_feed">
                <div class="container">
                    <div class="d-flex mb-4 align-items-baseline justify-content-between">
                        <h3 class="fs-16 fs-md-20 fw-700 mb-3 mb-sm-0">
                            <span>{{ translate('Collections Feed') }}</span>
                        </h3>
                        <div class="d-flex">
                            <a class="text-blue fs-12 fw-700 hov-text-primary" href="{{ route('frontend.showcase.collection') }}">{{ translate('View All') }}</a>
                        </div>
                    </div>

                    <div class="row gutters-16 row-cols-xl-4 row-cols-lg-4 row-cols-md-3 row-cols-2">
                        @foreach ($collectionFeedItems as $collectionItem)
                            @php
                                $collectionTitle = $locale === 'en'
                                    ? (data_get($collectionItem, 'title_en') ?: data_get($collectionItem, 'title_gr') ?: data_get($collectionItem, 'title') ?: translate('Collection'))
                                    : (data_get($collectionItem, 'title_gr') ?: data_get($collectionItem, 'title_en') ?: data_get($collectionItem, 'title') ?: translate('Collection'));

                                $collectionDescription = $locale === 'en'
                                    ? (data_get($collectionItem, 'description_en') ?: data_get($collectionItem, 'description_gr') ?: data_get($collectionItem, 'description'))
                                    : (data_get($collectionItem, 'description_gr') ?: data_get($collectionItem, 'description_en') ?: data_get($collectionItem, 'description'));

                                $collectionImage = data_get($collectionItem, 'cover_image');
                                $collectionImageUrl = null;

                                if (!empty($collectionImage)) {
                                    if (is_numeric($collectionImage)) {
                                        $collectionImageUrl = uploaded_asset($collectionImage);
                                    } elseif (filter_var($collectionImage, FILTER_VALIDATE_URL)) {
                                        $collectionImageUrl = $collectionImage;
                                    } else {
                                        $collectionImageUrl = asset($collectionImage);
                                    }
                                }

                                $collectionUrl = route('frontend.showcase.post', [
                                    'id' => data_get($collectionItem, 'id'),
                                    'slug' => \Illuminate\Support\Str::slug($collectionTitle ?: data_get($collectionItem, 'id')),
                                ]);
                            @endphp

                            <div class="col mb-4">
                                <div class="card h-100 border-0 shadow-sm overflow-hidden">
                                    <a href="{{ $collectionUrl }}" class="d-block text-reset">
                                        @if($collectionImageUrl)
                                            <img src="{{ $collectionImageUrl }}" alt="{{ $collectionTitle }}" class="img-fit w-100" style="height:220px;">
                                        @else
                                            <div class="d-flex align-items-center justify-content-center bg-light text-muted text-center" style="height:220px;">
                                                <div>
                                                    <div class="fw-700 mb-1">{{ translate('Collection') }}</div>
                                                    <div class="fs-12">{{ translate('No preview image') }}</div>
                                                </div>
                                            </div>
                                        @endif
                                    </a>
                                    <div class="card-body p-3">
                                        <h4 class="fs-14 fw-700 mb-2">
                                            <a href="{{ $collectionUrl }}" class="text-reset">
                                                {{ \Illuminate\Support\Str::limit($collectionTitle, 50) }}
                                            </a>
                                        </h4>
                                        @if(!empty($collectionDescription))
                                            <p class="fs-12 text-muted mb-0">
                                                {{ \Illuminate\Support\Str::limit(strip_tags($collectionDescription), 80) }}
                                            </p>
                                        @endif
                                    </div>
                                </div>
    ...
    <section class="mb-3 mt-3" id="section_types">
        <div class="container">
			...
			@php
    $shopCollectionFeedItems = \Illuminate\Support\Facades\DB::table('showcases')
        ->where('seller_id', $shop->id)
        ->where('status', 'published')
        ->where('type', 'collection')
        ->orderByDesc('created_at')
        ...
        ->map(function ($item) {
            ...
            $item->title = $locale === 'en'
                ? (($item->title_en ?? null) ?: ($item->title_gr ?? null) ?: ($item->title ?? null))
                : (($item->title_gr ?? null) ?: ($item->title_en ?? null) ?: ($item->title ?? null));

            $item->description = $locale === 'en'
                ? (($item->description_en ?? null) ?: ($item->description_gr ?? null) ?: ($item->description ?? null))
                : (($item->description_gr ?? null) ?: ($item->description_en ?? null) ?: ($item->description ?? null));

            return $item;
        });
@endphp

@if($shopCollectionFeedItems->count() > 0)
    <section class="mt-3 mb-3" id="section_showcase_collections">
        <div class="container">
            <div class="d-flex mb-4 align-items-baseline justify-content-between">
                <h3 class="fs-16 fs-md-20 fw-700 mb-3 mb-sm-0">
                    <span class="pb-3">{{ translate('Collections Feed') }}</span>
                </h3>

                <div class="d-flex">
                    <a type="button" class="arrow-prev slide-arrow text-secondary mr-2"
                       onclick="clickToSlide('slick-prev','section_showcase_collections')">
                        <i class="las la-angle-left fs-20 fw-600"></i>
                    </a>

                    <a class="text-blue fs-12 fw-700 hov-text-primary"
                       href="{{ route('frontend.showcase.collection') }}">
                        {{ translate('View All') }}
                    </a>

                    <a type="button" class="arrow-next slide-arrow text-secondary ml-2"
                       onclick="clickToSlide('slick-next','section_showcase_collections')">
                        <i class="las la-angle-right fs-20 fw-600"></i>
                    </a>
                </div>
            </div>

            <div class="aiz-carousel sm-gutters-16 arrow-none" data-items="4" data-xl-items="4" data-lg-items="3" data-md-items="2" data-sm-items="2" data-xs-items="1" data-arrows="true" data-infinite="false">
                @foreach($shopCollectionFeedItems as $collectionItem)
                    @php
                        $collectionImageUrl = null;

                        if (!empty($collectionItem->cover_image)) {
                            if (is_numeric($collectionItem->cover_image)) {
                                $collectionImageUrl = uploaded_asset($collectionItem->cover_image);
                            } elseif (filter_var($collectionItem->cover_image, FILTER_VALIDATE_URL)) {
                                $collectionImageUrl = $collectionItem->cover_image;
                            } else {
                                $collectionImageUrl = asset($collectionItem->cover_image);
                            }
                        }

                        $collectionUrl = route('frontend.showcase.post', [
                            'id' => $collectionItem->id,
                            'slug' => \Illuminate\Support\Str::slug($collectionItem->title ?: $collectionItem->id),
                        ]);
                    @endphp
                    ...
                    <div class="carousel-box">
                        <div class="card h-100 border-0 shadow-sm overflow-hidden">
                            ...
                            <div class="card-body p-3">
                                <h4 class="fs-14 fw-700 mb-2">
                                    <a href="{{ $collectionUrl }}" class="text-reset">
                                        {{ \Illuminate\Support\Str::limit($collectionItem->title ?: translate('Collection'), 55) }}
                                    </a>
                                </h4>

                                @if(!empty($collectionItem->description))
                                    <p class="fs-12 text-muted mb-0">
                                        {{ \Illuminate\Support\Str::limit(strip_tags($collectionItem->description), 90) }}
                                    </p>
                                @endif
                            </div>
                        </div>
