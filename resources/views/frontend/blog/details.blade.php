            ...
            <div class="col-xxl-7 col-lg-8">
                <div class="mb-4">
                    ...
                    @if(!empty($products) && $products->count() > 0)
                        <div class="mb-4">
                            @foreach($products as $product)
                                ...
                                @endphp
                                <div class="border rounded p-3 mb-3 d-flex flex-column flex-md-row align-items-md-center" style="gap:16px;">
                                    <a href="{{ $product_url }}" class="d-block overflow-hidden" style="width:140px;">
                                        <img
                                    ...
                                    <div class="flex-grow-1">
                                        <a href="{{ $product_url }}" class="d-block fw-700 fs-16 text-reset hov-text-primary">
                                            {{ $product->getTranslation('name') }}
                                        </a>
                                        <div class="mt-2">
                                            <span class="fw-700 text-primary">{{ home_discounted_base_price($product) }}</span>
                                            @if (home_base_price($product) != home_discounted_base_price($product))
                                                <del class="text-secondary ml-2">{{ home_base_price($product) }}</del>
                                            @endif
                                        </div>
                                        <button
                                            type="button"
                    ...
                    @if(!empty($blog->hashtags))
                        <div class="mb-4 blog-hashtags">
                            <div class="fw-700 mb-2">{{ translate('Hashtags') }}</div>
                            <div class="d-flex flex-wrap blog-hashtag-list">
                                @foreach(explode(',', $blog->hashtags) as $tag)
                                    @php $tag = trim($tag); @endphp
                                    @if($tag !== '')
                                        <span class="badge badge-soft-secondary blog-hashtag-item">#{{ $tag }}</span>
                                    @endif
                                @endforeach
                            </div>
                        </div>
