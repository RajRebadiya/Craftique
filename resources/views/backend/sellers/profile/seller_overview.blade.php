<div class="row">
    ...
    <div class="col-lg-7 d-flex flex-column gap-3">
        ...
        @if(!empty(array_filter([
            $business_info['business_role'] ?? null,
            $business_info['business_category'] ?? null,
            $business_info['product_quantity_range'] ?? null,
            $business_info['eshop_domain'] ?? null,
            $business_info['brand_count'] ?? null,
        ])))
        <div class="card rounded-2 border-color card-no-shadow mt-1">
            <div class="card-body p-0">
                <h6 class="card-title p-3 border-bottom font-weight-bold mb-0">{{translate('Business Details')}}</h6>

                <div class="p-3">
                    <div class="d-flex flex-column fs-13 gap-2">
                        <div><span class="font-weight-bold mb-1">{{translate('What are you?')}}</span>: <span>{{ $business_role_labels[$business_info['business_role'] ?? ''] ?? ($business_info['business_role'] ?? '-') }}</span></div>
                        <div><span class="font-weight-bold mb-1">{{translate('Category')}}</span>: <span>{{ $business_info['business_category'] ?? '-' }}</span></div>
                        <div><span class="font-weight-bold mb-1">{{translate('Products available')}}</span>: <span>{{ $business_info['product_quantity_range'] ?? '-' }} @if(!empty($business_info['product_quantity_exact'])) ({{ $business_info['product_quantity_exact'] }}) @endif</span></div>
                        <div><span class="font-weight-bold mb-1">{{translate('E-shop domain')}}</span>: <span>{{ $business_info['eshop_domain'] ?? '-' }}</span></div>
                        <div><span class="font-weight-bold mb-1">{{translate('Brand count')}}</span>: <span>{{ $business_info['brand_count'] ?? '-' }}</span></div>
                        <div><span class="font-weight-bold mb-1">{{translate('Terms accepted')}}</span>: <span>{{ !empty($business_info['accept_seller_terms']) ? translate('Yes') : translate('No') }}</span></div>
                    </div>
                </div>
            </div>
        </div>
